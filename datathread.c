...
void datathread_on_arq(char *data, size_t size)
{
    ...
    case ST_ARQ_FILE_RCV:
        ...
        datathread_queue_heard(inbuffer);
        arim_on_arq_file_rcv_frame(data, size);
        break;
    case ST_ARQ_MSG_RCV:
        arim_copy_remote_call(remote_call, sizeof(remote_call));
        snprintf(inbuffer, sizeof(inbuffer), "7[@] %-10s ", remote_call);
        datathread_queue_heard(inbuffer);
        arim_on_arq_msg_rcv_frame(data, size);
        break;
    ...
    case ST_ARQ_MSG_SEND:
        ...
        if (data[0] == '/')
            arim_on_arq_cmd(data, size);
        else
            arim_on_arq_resp(data, size);
        break;
    default:
...
void *datathread_func(void *data)
{
    ...
    while (1) {
        ...
        case 0:
            ...
            arim_on_arq_cmd(NULL, 0);
            arim_on_arq_resp(NULL, 0);
            break;
        case -1:
