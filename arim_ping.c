...
int arim_send_ping(const char *repeats, const char *to_call, int event)
{
    ...
    if (!g_tnc_attached || !arim_is_idle() || !repeats || !to_call)
        return 0;
...
int arim_send_ping_ack()
{
    ...
    snprintf(buffer, sizeof(buffer),
             "<< [p] %s>%s S/N: %sdB, Quality: %s",
                ping_tcall, ping_scall, db > 20 ? ">20" : ping_sn, ping_qual);
    ui_queue_traffic_log(buffer);
    if (!strncasecmp(g_ui_settings.mon_timestamp, "TRUE", 4)) {
        snprintf(buffer, sizeof(buffer),
...
int arim_recv_ping_ack(const char *data)
{
    ...
    if (sn && qual) {
        ...
        if (atoi(qual) < atoi(g_arim_settings.pilot_ping_thr)) {
            status = -1;
            snprintf(buffer, sizeof(buffer), "PP: Send canceled, PINGACK quality %s below threshold %s",
