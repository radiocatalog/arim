...
int cmdproc_cmd(const char *cmd)
{
    ...
    case ST_ARQ_CONNECTED:
        if (!strncasecmp(cmd, "/DIS", 4)) {
            ...
            if (result1 == 'y' || result1 == 'Y')
                arim_send_arq_disconn_req();
        } else {
            if (!strncasecmp(cmd, "/FGET", 5)) {
                arim_on_arq_file_get();
                ...
                if (*destdir == '>')
                    *destdir++ = '\0';
                    ...
                    destdir = NULL;
                arim_on_arq_file_put(fn, destdir, zoption);
                return 1;
                ...
                if (!t) {
                    ...
                    if (ui_create_msg(msgbuffer, sizeof(msgbuffer), buffer))
                        arim_on_arq_msg_put(msgbuffer, zoption);
                } else {
    ...
    default:
        ...
        if (t && strlen(t) == 2 && !strncasecmp(t, "li", 2)) {
            ...
            if (!arim_send_arq_conn_req(t, buffer))
